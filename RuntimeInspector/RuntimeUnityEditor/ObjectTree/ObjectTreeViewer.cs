...
namespace RuntimeUnityEditor.Core.ObjectTree
{
    public sealed class ObjectTreeViewer
    {
        ...
        private void DisplayObjectTreeHelper(GameObject go, int indent, ref int currentCount)
        {
            ...
            var isVisible = currentCount * _singleObjectTreeItemHeight >= _scrollPosY &&
                            (currentCount - 1) * _singleObjectTreeItemHeight <= _scrollPosY + _objectTreeHeight;

            if (go && (isVisible || needsHeightMeasure || _scrollTreeToSelected))
            {
                var c = GUI.color;
			...
			if( go && _openedObjects.Contains( go ) )
			{
        ...
        // Keeping scoll area's height and scrollbar's y-position constant during the frame helps
        // avoid ArgumentExceptions in DisplayObjectTreeHelper
        private int _scrollPosFrame;
        private float _scrollPosY;
        private float _scrollAreaSize;
        ...
        private void DisplayObjectTree()
        {
            GUILayout.BeginVertical(GUI.skin.box);
            {
                ...
                _treeScrollPosition.y = Mathf.Min( _treeScrollPosition.y, _scrollAreaSize );
                {
                    GUILayout.BeginVertical();
                    {
                        var currentCount = 0;
                ...
                if( Time.frameCount != _scrollPosFrame && Event.current.type == EventType.Repaint )
                {
                    _scrollPosFrame = Time.frameCount;
                    _scrollAreaSize = GUILayoutUtility.GetLastRect().height;
                    _scrollPosY = _treeScrollPosition.y;
                }
